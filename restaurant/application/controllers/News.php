...
class News extends CI_Controller {
	...
	public function addCateNews()
	{
		...
		$this->load->model('news_model');
		if($this->news_model->insertCateNews($tendanhmuc, $status)) {
	...
	{ 
		...
		if($this->news_model->updateCateById($id, $name_catenews, $status)) {
			$this->load->view('thanhcongnews');
		} else {
			$this->load->view('thatbainews');
		}
	...
	public function addJquery()
	{
		...
		$status = $this->input->post('status');
		$this->load->model('news_model');
		$this->news_model->insertCateNews($tendanhmuc, $status);
	...
	public function editNews($id)
	{
		...
		$tendanhmuc = $this->news_model->getDataCateNews($id);
		$ketqua = array(
			'dulieusua' => $ketqua,
			'tendanhmuc' => $tendanhmuc
		);
		$this->load->view('editNews_view', $ketqua, FALSE);
	}
	public function updateNews()
	{
		// dựa vào id để lấy tin
		$image_newsold = $this->input->post('image_newsold');
		$image_news = $_FILES['image_news']['name'];
		// nếu không up ảnh mới
		if (empty($image_news)) {
			$image_news = $image_newsold;
		...
		if($this->news_model->editNewsById($id, $title, $id_catenews, $content_news, $image_news, $quote)) {
			$this->load->view('thanhcong2');
		} else {
			$this->load->view('thatbai2');
		}
