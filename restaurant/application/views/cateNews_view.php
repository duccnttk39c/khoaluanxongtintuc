		...
		<div class="col-sm-6">
			...
			<!-- <form action="news/addCateNews" method="POST"> -->
				<fieldset class="form-group">
					<label for="formGroupExampleInput">Tên danh mục tin</label>
			    ...
			    <div class="card-block">
				    ...
				    <h4 class="card-blockquote">
			    		<fieldset class="form-group jquery_tendanhmuc hidden-xs-up">
			    			...
			    			<input type="hidden" class="form-control status" value="<?= $value['status']; ?>">
			    		</fieldset>
	...
	$(function() {
		...
		$('#nutthemdanhmuc').click(function(event) {
			...
			$.ajax({
				...
				dataType: 'json',
				data: {
					name_catenews:  $('#name_catenews').val(),
					status: $('#status').val()
			},
			})
			.done(function() {
			...
			.always(function(res) {
				...
				noidung+= '<input type="text" class="form-control tendanhmucsua" value="'+$('#name_catenews').val()+'">';
				noidung+= '<input type="hidden" class="form-control status" value="'+$('#status').val()+'">';
				noidung+= '</fieldset>';
